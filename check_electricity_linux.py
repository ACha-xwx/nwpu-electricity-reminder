...
async def wait_for_api(auth, timeout_seconds=120, retry_interval=5):
    ...
    while True:
        if time.time() - start_time > timeout_seconds:
            raise TimeoutError(f"等待校园电费接口超时，最后一次错误：{last_error}")

        try:
            async with create_async_client(auth, timeout=15.0) as client:
            ...
            last_error = exc
            print(f"校园接口暂时不可用：{exc}，{retry_interval} 秒后重试。")
            await asyncio.sleep(retry_interval)


def write_log(base_dir, electric_left, room_info, current_time):
    with (base_dir / "info.log").open("a", encoding="utf-8") as file:
        file.write(f"{current_time} | {electric_left} | {room_info}\n")
...
def send_notifications(message, config, base_dir=None, source="scheduled_check"):
...
async def main():
    ...
    file_path = get_config_path(base_dir)
    ...
    try:
        await wait_for_api(auth)

        if not file_path.exists():
            print("配置文件不存在，请先运行 capture_web_session.py。")
            return
        ...
        write_log(base_dir, electric_left, room_info, current_time)
